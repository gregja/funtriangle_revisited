/*
* Intermediate version where the drag & drop mode is managed by red balls drawn in SVG (with
* event listeners affected to the red balls),
* triangle and circles are drawn in the canvas, and the red balls too (redrawn for rendering
* of the PNG image saved)
*
* Version intermédiaire dans laquelle le mode drag & drop est géré via des écouteurs d'événement
* affectés aux boules rouges dessinées en SVG, les triangles et cercles sont dessinés
* dans le canvas, de même que les boules rouges, dessinées elles aussi dans le canvas pour
* le rendu de l'image PNG sauvegardée
*/
var trianglefunModule = (function () {
    ...
    */
    ...
    Triangle.prototype.update=function(){
        //clearCanvas();  // dropped unit clearing (transfered in "updateAll" function)
      ...
      for(let i=3;i--;){
        draw("circle",{
            "class":"red-ball",
            "cx":t.p[i].el.attributes['cx'].value,
            "cy":t.p[i].el.attributes['cy'].value,
            "r" :13
        })
